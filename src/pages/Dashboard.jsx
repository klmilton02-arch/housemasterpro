...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-3 max-w-md md:max-w-2xl mx-auto px-2 sm:px-1 pt-7 text-black dark:text-black [&_*]:text-black [&_p]:text-black [&_h1]:text-black [&_h2]:text-black [&_span]:text-black" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div className="grid gap-3 sm:gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
          ...
          <StatCard large={largeIcons} icon={Clock} label="Pending" value={pendingTasks.length} color="bg-amber-100 text-amber-600" onClick={() => { setDrawerTaskIds(pendingTasks.map(t => t.id)); setTaskListModal({ title: 'Pending Tasks' }); }} />
      ...
      <div className="grid grid-cols-1 gap-3 sm:gap-4">
          ...
          <Link to="/family" className="block">
            <StatCard large={largeIcons} labelRight icon={ListChecks} label="Family" value="View" color="bg-pink-100 text-pink-600" bigText />
          </Link>
          </div>
