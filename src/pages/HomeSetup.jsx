...
export default function HomeSetup() {
  ...
  async function generateTasks() {
    ...
    try {
      ...
      const result = await base44.functions.invoke('generateHomeSetupTasks', { taskData });
      setGenerated(result.data?.created || 0);
      } catch (error) {
    ...
    <div className="space-y-7 max-w-sm md:max-w-2xl mx-auto px-3 sm:px-2 pt-7" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="space-y-3">
          <button onClick={() => setBedroomModalOpen(true)} className="h-22 w-full">
            <StatCard icon={BedDouble} label={config.bedrooms === 1 ? "Bedroom" : "Bedrooms"} value={config.bedrooms} color="bg-blue-100 text-blue-600" labelRight large={largeIcons} bigText />
          </button>
          <button onClick={() => setBathroomModalOpen(true)} className="h-22 w-full">
            <StatCard icon={Bath} label="Full Bathrooms" value={config.full_bathrooms} color="bg-purple-100 text-purple-600" labelRight large={largeIcons} bigText />
          </button>
          <button onClick={() => setHalfBathroomModalOpen(true)} className="h-22 w-full">
            <StatCard icon={Bath} label="Half Bathrooms" value={config.half_bathrooms} color="bg-pink-100 text-pink-600" labelRight large={largeIcons} bigText />
          </button>
        </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setConfig({ bedrooms: 2, full_bathrooms: 1, half_bathrooms: 0, has_kitchen: true, has_living_room: true, has_dining_room: false, has_garage: false, has_laundry_room: false, has_mixed_use: false, has_office: false, has_whole_house: false })} className="flex-1 h-12 text-sm">
            Reset
          </Button>
