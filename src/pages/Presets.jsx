...
function PresetCard({ p, onClick }) {
  ...
  return (
    <button
      onClick={() => onClick(p)}
      className="w-full bg-card border border-border hover:shadow-md hover:border-primary/30 rounded-xl px-3 py-2 transition-all flex items-center gap-3 text-left h-14 active:scale-95"
    >
      {style.emoji && (
    ...
    <div className="space-y-7 max-w-sm md:max-w-2xl mx-auto px-3 sm:px-2 pt-7" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          <Button onClick={() => { setEditingPreset(null); setEditDialogOpen(true); }} className="gap-2 w-full h-14 text-base font-medium bg-primary hover:bg-primary/90">
