...
Deno.serve(async (req) => {
  try {
    ...
    try {
      console.log(`Sending invite email to ${email} with code ${invite_code}`);
      const emailResult = await base44.integrations.Core.SendEmail({
        from_name: 'HomeLifeFocus',
        to: email,
      ...
      return Response.json({ error: `Email sending failed. Check your email settings.` }, { status: 500 });
    }
